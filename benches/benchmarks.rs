...
pub fn row_meanvar(c: &mut Criterion) {
    let nm = StandardMat {
        nrows: BENCH_BLOCK_ROWS,
        ncols: BENCH_BLOCK_COLS,
    };
    let sample: Mat<f32> = nm.sample(&mut rand::thread_rng());
    c.bench_function("row_meanvar", |b| {
        b.iter(|| math::row_meanvar(black_box(sample.as_ref())))
    });
}
...
pub fn column_meanvar(c: &mut Criterion) {
    let nm = StandardMat {
        nrows: BENCH_BLOCK_ROWS,
        ncols: BENCH_BLOCK_COLS,
    };
    let sample: Mat<f32> = nm.sample(&mut rand::thread_rng());
    c.bench_function("column_meanvar", |b| {
        b.iter(|| math::column_meanvar(black_box(sample.as_ref())))
    });
}
